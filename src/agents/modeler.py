...
def _generate_class_name(problem_name: str) -> str:
    ...
    Java identifiers must start with a letter or underscore. Names like
    "4 Queens" would otherwise produce `4QueensSolver` which fails to compile
    and breaks the whole refinement loop because the file path is locked in.
    """
    words = re.sub(r'[^a-zA-Z0-9\s]', '', problem_name).split()
    name = "".join(w.capitalize() for w in words) + "Solver"
    if not name or not name[0].isalpha():
        name = "P" + name  # safe prefix; keeps stable across runs
    return name

...
def compile_and_run_model(java_code: str, class_name: str) -> dict:
    ...
    # Maven compiles every .java in this dir. Stale files from prior runs will
    # fail the build even if the current run's code is correct. Wipe everything
    # except DynamicRunner.java (the persistent smoke-test utility).
    for fname in os.listdir(source_dir):
        if fname.endswith(".java") and fname != "DynamicRunner.java":
            try:
                os.remove(os.path.join(source_dir, fname))
            except OSError:
                pass

    # CRITICAL: also wipe stale .class files from target/classes/runner/.
    # Maven's incremental compiler may decide a truncated/invalid .java doesn't
    # need recompilation, then mvn exec:java runs the OLD .class from a
    # previous successful run — fooling the pipeline into reporting bogus
    # "success" with another problem's solver output.
    classes_dir = os.path.join(CHOCO_RUNNER_DIR, "target", "classes", "runner")
    if os.path.isdir(classes_dir):
        for fname in os.listdir(classes_dir):
            if fname.endswith(".class") and fname != "DynamicRunner.class":
                try:
    ...
    try:
        ...
        if compile_proc.returncode != 0:
            ...
            # Maven writes [ERROR] file:line: ... lines to STDOUT, not stderr.
            # Combine both so the captured error_message has the real diagnosis.
            result["stderr"] = (

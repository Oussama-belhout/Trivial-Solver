"""L3 PDA — full data analysis across 3 models × 1 prompting variant.

Inputs:  results/L3/L3_few_shot_{model}.json   (3 files)
Outputs: results/L3/L3_analysis/
           summary_wide.csv         — one row per model, all metrics
           per_problem.csv          — one row per (model, problem)
           summary.md               — narrative analysis
           plots/*.png              — visuals

L3 corresponds to the "Formalizer split" rung: four specialized roles
(Formalizer + Modeler + Validator + Refiner). The Formalizer emits a
Pydantic-validated CSPSpecification, the Validator runs on the formal spec,
and the Refiner remains Reflexion-style.
...
def load_run(model: str) -> dict:
    p = LEVEL_DIR / f"L3_few_shot_{model}.json"
...
grouped_bar("success_rate", "Success rate (%)", "L3 — Success rate by model", "01_success_rate.png", ymax=110)

grouped_bar(
    "first_shot_rate",
    "First-shot success rate (%)",
    "L3 — First-shot success rate (no refinement)",
    "02_first_shot.png",
    ymax=110,
)
...
grouped_bar(
    ...
    "L3 — Compilation failure rate",
    "03_compile_fail.png",
    ymax=max(50, wide["compilation_failure_rate"].max() + 5),
)

grouped_bar(
    "mean_iterations",
    "Mean refinement iterations",
    "L3 — Mean refinement iterations per problem",
...
ax.set_title("L3 — Success rate by problem difficulty")
...
for i in range(cat_heat.shape[0]):
    for j in range(cat_heat.shape[1]):
        ...
        ax.text(j, i, f"{v:.0f}" if not np.isnan(v) else "-", ha="center", va="center",
                color="black" if 30 < v < 75 else "white", fontsize=9)
ax.set_title("L3 — Per-category success rate")
...
ax.set_title("L3 — Distribution of refinement iterations per model")
ax.grid(axis="y", linestyle="--", alpha=0.4)
...
ax.set_xlabel("Mean wall time per problem (s, log scale)")
ax.set_ylabel("Success rate (%)")
ax.set_title("L3 — Cost vs. quality (size = first-shot rate)")
ax.set_xscale("log")
...
ax.set_title("L3 — Faithfulness (regex)")
ax.set_ylim(0, 100)
ax.grid(axis="y", linestyle="--", alpha=0.4)
fig.tight_layout()
...
if len(unique_stats) <= 1 and len(best_runs_full) > 0:
    fig.suptitle("L3 — Solver-side stats per problem [POSSIBLY CORRUPT]", y=1.03, color="darkred")
    fig.text(
        0.5,
        0.965,
    ...
    fig.suptitle("L3 — Solver-side stats per problem", y=1.03)
...
md = []
md.append("# L3 PDA — Data Analysis\n")
...
md.append("- Variant: few-shot (Formalizer + Modeler + Validator + Refiner).")
md.append("- Benchmarks: 4 hard / 3 medium / 3 easy across 8 CSP families.")
md.append("- L3 topology: Formalizer → Modeler → Validator → Solver, with Reflexion-style Refiner re-entering at Validator.")
...
md.append("### F1 — Qwen is the L3 winner (90% success), reversing its L1 collapse")
md.append("Qwen reaches **90% success** with **10% compilation failures** and a mean of **1.8 iterations**. "
         "Compared to its 0%/100%-compile-fail collapse at L1, the L3 split is the rung where Qwen finally fits the workflow: "
         "the Formalizer's structured-output stage absorbs the response-shape mismatch that previously broke the Refiner-only "
         "L1 setup, and the spec-grounded Validator catches the modeling errors before they reach the solver. "
         "The single failure (scheduling) is consistent with the family's known semantic difficulty rather than a Qwen-specific defect.\n")

md.append("### F2 — Haiku regresses from L1 (100%) to L3 (60%) on hard problems")
md.append("Haiku drops to **60% success**, **0% first-shot**, **30% compilation failures**, and **1.9 mean iterations**. "
         "The breakdown is sharp by difficulty: easy/medium remain at 100% but **all 4 hard problems fail** "
         "(sudoku, cryptarithmetic, scheduling, knapsack). The first-shot rate falling to 0% indicates that the new "
         "Formalizer/Validator gate now rejects the initial Modeler output even when the underlying Java would have compiled, "
         "while the validator/refiner loop fails to recover the fix within the iteration budget on hard inputs. "
         "This is the clearest example in the PDA sweep of **role-splitting hurting an already-strong model**: the extra "
         "spec-conformance pressure is friction that L1's lighter pipeline did not impose.\n")

md.append("### F3 — DeepSeek's wall time explodes ~16× while success drops to 50%")
md.append("DeepSeek lands at **50% success**, **40% first-shot**, **40% compilation failures**, and a mean wall time of "
         "**~2241 seconds per problem** (vs ~139s at L1 — a ~16× cost increase). The deep-reasoning trace at every node "
         "(Formalizer, Modeler, Validator, Refiner) compounds, and each refinement iteration repays that cost. "
         "Despite the budget the model still trails Qwen on the same 10 problems and trails its own L1 result (80% → 50%). "
         "At L3 DeepSeek is dominated on both axes (success and cost) and is no longer a defensible default.\n")

md.append("### F4 — Hard problems separate models more aggressively than at L1")
md.append("By difficulty, Qwen sustains 75% on hard, Haiku falls to 0%, and DeepSeek lands at 50%. The hard tier (sudoku, "
         "cryptarithmetic, scheduling, knapsack) is where the Formalizer split's added validation friction is most visible: "
         "the spec must be both correct and faithful to the NL description, and the Validator's spec-level checks have less "
         "slack than the L1 NL-grounded validator did. Models that the Formalizer cannot reliably guide (Haiku here) lose "
         "headroom that L1 hid.\n")

md.append("### F5 — Faithfulness remains at 0% (parser dependency)")
md.append("Faithfulness regex pass rate is 0% across all models (mean overlap 0.0). As in L0/L1, the explanation metric "
         "is not meaningful until the parser populates the `solution` dict with variable assignments — this is an "
         "infrastructure gap, not a model deficiency.\n")

md.append("## Cross-cutting observations\n")
md.append("- **Role-splitting helps weak baselines, hurts strong ones.** Qwen gains the most from L3 (0% → 90%); Haiku, the "
          "L1 leader, loses 40 points. Treat L3 as a model-dependent rung, not a strict upgrade.")
md.append("- **First-shot rates collapse across the board** vs L1 (Qwen 0→30%, Haiku 70→0%, DeepSeek 40→40%). The Formalizer "
          "gate effectively rules first-shot success out for Haiku and forces refinement-driven recovery to do the work.")
md.append("- **DeepSeek wall time is the headline cost.** ~37 minutes per problem on average makes it impractical at L3 "
          "absent specific reasons to need it; report wall time alongside success in any cross-level comparison.")
md.append("- **Compile failures stay non-zero everywhere.** L3 does not eliminate the syntactic-failure mode that L1's "
          "Refiner partially fixed for Haiku — the spec layer adds correctness pressure but does not replace a guard "
          "for malformed Java.")
md.append("- **Hard categories (cryptarithmetic, scheduling) remain the discriminator.** Sudoku now also separates models "
          "at L3 because the spec-level Validator is stricter on grid encodings.")
md.append("")
